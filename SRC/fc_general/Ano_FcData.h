...
typedef struct
{
	...
	//时间记录
	u8 time_take_off;//拨一键起飞时的系统时间
	u8 time_turn_360;//拨键开始旋转的时间
	u8 opmv_distance;//openmv测得距离经滤波后的值
...
typedef struct
{
	u32 take_off;//起飞时间储存变量
	u8 take_off_flag;//0代表未存储，1代表已经存储
	u32 start_turn;//开始绕杆时间存储变量
	u8 start_turn_flag;//0代表未存储，1代表已经存储
	u32 start_move;
	u8 start_move_flag;
