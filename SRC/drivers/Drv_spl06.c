...
float Drv_Spl0601_Read ( void )
{
	...
	spl_data.temperature = spl0601_get_temperature();

	spl0601_get_raw_pressure();
	spl_data.baro_pressure = spl0601_get_pressure();

    //alt_high = powf((temp/101325),1/5.255f);
/////////////////////////////////////////////////////////////

	alt_3 = ( 101400 - spl_data.baro_pressure ) / 1000.0f;
	spl_data.height = 0.82f * alt_3 * alt_3 * alt_3 + 9.0f * ( 101400 - spl_data.baro_pressure );
	spl_data.height_comp = spl_data.height + LIMIT(BARO_COMP,-300,0);
	//
	if(dly_cnt!=0)
	{
		dly_cnt--;
